...
public class Camera2VideoFragment extends Fragment
    ...
    /**
     * In this sample, we choose a video size that can support high speed video mode such as 120fps
     *
    ...
    private Size chooseVideoSize(StreamConfigurationMap map) {
        ...
        for (Range<Integer> fpsRange : map.getHighSpeedVideoFpsRanges()) {
            if (fpsRange.getLower().equals(fpsRange.getUpper())) {
                for (Size size : map.getHighSpeedVideoSizesFor(fpsRange)) {
                    if (isHasHighSpeedCamcorder(size, CameraMetadata.LENS_FACING_FRONT)) {
                        Log.d(TAG, "Support HighSpeed video recording for " + size.getWidth() + "x" + size.getHeight() + " " + fpsRange.getLower() + "fps");
                        highSpeedSizes.add(size);
                    }
                }
            }
        }
        ...
        Collections.sort(highSpeedSizes, new CompareSizesByArea());
        return highSpeedSizes.get(highSpeedSizes.size() - 1);
    }

    private static boolean isHasHighSpeedCamcorder(Size size, int cameraID) {
        if (size.getWidth() == 720 && size.getHeight() == 480) {
            ...
            return size.getWidth() == 3840 && size.getHeight() == 2160 && CamcorderProfile.hasProfile(cameraID, CamcorderProfile.QUALITY_HIGH_SPEED_2160P);
    }
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        ...
        mInfo        = view.findViewById(R.id.video_info);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.video: {
                ...
                if (mIsRecordingVideo) {
                    stopRecordingVideo();
                    SlowMotionThread slowMotionThread = new SlowMotionThread(activity, mNextVideoAbsolutePath);
                    new Thread(slowMotionThread).start();
                } else {
                    startRecordingVideo();
                }
                break;
    ...
    private void openCamera(int width, int height) {
        ...
        try {
            ...
            for (int i = 0; i < manager.getCameraIdList().length; i++) {
                Log.d(TAG, "cameraID " + manager.getCameraIdList()[i]);
            }
            ...
            if(null == mVideoSize) return;
            mPreviewSize = chooseOptimalSize(map.getOutputSizes(SurfaceTexture.class),
                    width, height, mVideoSize);
            Log.d(TAG, "mPreviewSize " + mPreviewSize.getWidth() + "x" + mPreviewSize.getHeight());

            for(Range<Integer> fpsRange : map.getHighSpeedVideoFpsRangesFor(mVideoSize)) {
                if (fpsRange.getLower().equals(fpsRange.getUpper())) {
                    mFPS = fpsRange.getLower();
                }
            }
            mInfo.setText(getString(R.string.video_info, mVideoSize.getWidth(), mVideoSize.getHeight(), mFPS));
...
//            thread.start();
    ...
    private void setUpCaptureRequestBuilder(CaptureRequest.Builder builder) {
        if(mIsRecordingVideo) {
            Range<Integer> fpsRange = Range.create(mFPS, mFPS);
...
//                + System.currentTimeMillis() + ".mp4";
        return Environment.getExternalStorageDirectory().getAbsolutePath() + "/Movies/video.mp4";
    }
...
//            }
        ...
        if(mPreviewSessionHighSpeed != null) {
            mPreviewSessionHighSpeed.close();
...
//        mNextVideoAbsolutePath = null;
    ...
    public static class SlowMotionThread implements Runnable {

        private Activity mActivity;
        private String mFilePath;
        private MediaCodecProc mMediaCodec;

        SlowMotionThread(Activity activity, String filePath) {
            mActivity = activity;
            mFilePath = filePath;
            mMediaCodec = new MediaCodecProc(mFilePath, 120, 4);
        }

        @Override
        public void run() {
            Looper.prepare();
            long t1 = Utils.getCurrentTimeMillis();
            mMediaCodec.process();
            long t2 = Utils.getCurrentTimeMillis() - t1;
            Toast.makeText(mActivity, "Slow motion processing is complete, time consuming" + t2 + "ms", Toast.LENGTH_SHORT).show();
            Log.d(TAG, "Slow motion processing is complete, time consuming" + t2 + "ms");
            Looper.loop();
        }
    }
}
